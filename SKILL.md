---
name: lieflat-charts
description: 一套模板驱动的数据可视化 skill，严格从 Lupi、Basics、Glance 与 Interactive gallery 的真实实现生成 HTML 图表；以 Mono 为保底，并能按数据语义和使用场景自动选择一套彩色预设，同一交付禁止混用色系。
---
...
颜色无需用户显式触发。按第六点五节在 Mono 与 `color-presets.js` 的三套内置方案之间自动判断；同一份 HTML 或同一组交付只能使用一种色系，不得跨预设混用。换肤样张在 `templates/color/`。
...
彩色初稿从三套预设中选择一套，色值在 `color-presets.js`，样张在 `templates/color/`。预设用于快速得到稳定、统一的第一版，不是锁死的色板。用户在生成后要求"这个蓝再深一点"或提供自定义色值时，可以在当前色系内继续调整；不得引入另一套预设的色值。调整后必须重新检查对比度、视觉主次和数据语义。

**同一份 HTML 或同一组交付必须先锁定一个全局色系：Mono、porcelain、palm、wire 四选一。** 所有图共享该选择。若其中某张图不支持已选预设，不得单独换另一套；改选一套全局兼容的色系，或整组退回 Mono。
...
- **同一交付只用一种色系。** 一张图、同一份 HTML 或同一组姊妹图只能使用 Mono 或 `color-presets.js` 的一套预设。禁止 porcelain、palm、wire 彼此混用。用户要求调整时，只能在当前色系内修改色值，并重新检查对比度、视觉主次和数据语义。
...
- **发光 / 玻璃拟态 / 3D** → 拒绝，保持现有视觉语法。自定义色值可以在当前色系内调整；跨预设混用拒绝。
...
2. 色板是否受控？整份 HTML 或同一组交付是否只使用 Mono 或单一预设？Mono 中出现任何非 ladder 颜色即返工；彩色中出现另一套预设的颜色也返工。若用户调整过颜色，是否仍保持同一色系，并重新检查对比度、视觉主次和数据语义？

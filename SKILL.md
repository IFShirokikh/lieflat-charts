---
name: lieflat-charts
description: 一套模板驱动的数据可视化与报告生成 skill，既能严格从 Lupi、Basics、Glance 与 Interactive gallery 的真实实现生成 HTML 图表，也能从 12 套中英文整页报告模板生成可发布的 HTML 报告；以 Mono 为保底，能按数据语义自动选择内置彩色预设，也支持用户明确提供的自定义色板，同一交付禁止混用色系。
---

# Lieflat Charts — 图表品味法典

Lieflat Charts 是一套遵循 Agent Skills 格式的数据可视化与报告生成 skill，以 Mono 灰阶为保底，也会在数据语义和使用场景明确适配时自动选择一套彩色预设。它支持 Lupi 编辑叙事型、Glance 快速判断型、Basics 基础编辑型、Interactive 交互大图，以及 `report-catalog.md` 中的 12 套中英文整页报告模板。用户给你数据和场合，可以得到一个无需构建、双击可打开的单文件 HTML 图表或 HTML 报告。纯 SVG 图可离线运行；使用 Chart.js、ECharts 或在线字体的图，在未内联依赖时需要联网。**默认必须先判断用户要单张图 / 图表页，还是完整报告；报告模式先从报告模板中选整页骨架，再为每个图表槽位复用 catalog 中的真实图型。**
...
- **图表模式**：用户要一张图、几张图、一个 dashboard 图表页、PPT 插图或可复用 chart block。按本文件原有 Lupi → Basics → Glance → Interactive 流程选图。
- **报告模式**：用户说“生成报告 / 报告模板 / 年报 / 月报 / 白皮书 / 调研一页纸 / 海报 / brief / notebook / dashboard 报告”，或希望把多个结论组成一页完整叙事。先读 `report-catalog.md`，按场景、版心、密度、阅读速度和语言选择一个 R01–R12。
...
3. **默认选型顺序固定。** 先完整比较 Lupi Editorial（L1–L15），再比较 Lupi Basics（F1–F13）。只要其中存在能诚实承载数据、容纳标签且可读的模板，就必须从这两组中选择。
...
2. **先审计 Lupi 与 Basics。** 按数据形状扫描所有能承接它的 Lupi Editorial（L1–L15）与 Lupi Basics（F1–F13）候选。至少比较 3 个候选；如果不足 3 个，就列出全部候选。比较语义契合、单位诚实、标签容纳、阅读速度、叙事张力和本批次是否重复。
...
1. 读取 `report-catalog.md`，至少比较 3 个报告候选并记录淘汰理由。
...
1. **先全量扫描，再选代码骨架**——基础型组 F1–F13（`templates/basics-gallery.html`）是稀疏数据的 Lupi 词汇：柱/折线/面积/环形/横条/分组/堆叠/散点/瀑布/热力/进度/哑铃/矩形树图；编辑型 L1–L15 则提供逐记录、单位分解、关系与旁注语法。不要因为 `basics-gallery.html` 里有现成代码就直接选 F 图。
...
用户要的图不在 49 张里（或发来一张参考图）时，**不是做不了，是现场造句**。流程四步，第一步不许跳：
